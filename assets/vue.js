...
new Vue( {
    ...
    methods: {
        ...
        search_blur () {
            if ( this.search === '' ) setTimeout( () => this.icons_search = true, 200 );
        },
        search_icons () {
            this.icons_search = !this.icons_search;
            if ( !this.icons_search ) this.focuss();
            if ( this.icons_search && this.search !== '' ) this.search = '';
        },
        last_message_users (user) {
            return user.messages.filter( e => e.status === 'sent' )[--this.last_message_users.length].date;
        }
    },
    computed: {
        search_icon () {
